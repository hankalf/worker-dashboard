...
// CSV columns: name (required), description (optional). "Roles" = Capability.
export async function POST(req: Request) {
  ...
  let updated = 0;
  const errors: string[] = [];
  let nextSort = await prisma.capability.count();
  ...
  if (created || updated) {
    await logActivity("Role", `Imported roles (${created} new, ${updated} updated)`);
  }
